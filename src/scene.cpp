...
void scene_structure::initialize()
{
	...
	rigid_body_square.mesh.initialize_data_on_gpu(cgp::mesh_primitive_quadrangle({ -0.1f, -0.1f, 0.0f }, { 0.1f, -0.1f, 0.0f }, { 0.1f, 0.1f, 0.0f }, { -0.1f, 0.1f, 0.0f }));
	rigid_body_square.mesh.model.scaling = 0.5f; // Adjust the size as needed.
	rigid_body_square.mesh.model.translation = { 0.0f, 0.5f, 0.0f };
	rigid_body_square.mesh.material.color = { 1, 0, 0 }; // Setting the color to blue
...
void scene_structure::display_frame()
{
	...
	for (int k = 0; k < particles.size(); ++k) {
		...
		bool collision_x = p.x >= (square_center.x - square_half_side) && p.x <= (square_center.x + square_half_side);
		bool collision_y = p.y >= (square_center.y - square_half_side) && p.y <= (square_center.y + square_half_side);
